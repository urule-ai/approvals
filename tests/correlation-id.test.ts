...
describe('approvals — x-correlation-id propagation via real buildServer', () => {
  it('echoes the inbound x-correlation-id header on the response', async () => {
    const app = await buildServer();
    const res = await app.inject({
      method: 'GET',
      url: '/healthz',
      headers: { 'x-correlation-id': 'test-approvals-abc-123' },
    });
    expect(res.statusCode).toBe(200);
    expect(res.headers['x-correlation-id']).toBe('test-approvals-abc-123');
    await app.close();
  });
