...
// Mirror approvals' server.ts publicRoutes config. Drift in either
...
async function buildAuthClosedApp() {
  ...
  app.post('/api/v1/approvals', async () => ({ id: 'a1' }));
  return app;
}

describe('approvals — fail-closed auth wiring', () => {
  it('returns 401 on a protected route with no auth header', async () => {
    const app = await buildAuthClosedApp();
    const res = await app.inject({ method: 'POST', url: '/api/v1/approvals', payload: {} });
